...
class InventoryManager: ObservableObject {
    ...
    func loadVehicles() async {
        do {
            let descriptor = FetchDescriptor<AppVehicle>(sortBy: [SortDescriptor(\.make), SortDescriptor(\.model), SortDescriptor(\.year)])
            vehicles = try modelContext.fetch(descriptor)
        } catch {
            logger.error("Failed to load vehicles: \(error.localizedDescription)")
    ...
    func createWarehouse(name: String, address: String, description: String = "") {
        let warehouse = AppWarehouse(id: UUID().uuidString, name: name, location: address, warehouseDescription: description)
    ...
    // Get inventory value for a specific warehouse
    func getInventoryValue(for warehouse: AppWarehouse) -> Double {
        guard let stockItems = warehouse.stockItems else { return 0.0 }
        
        return stockItems.reduce(0.0) { total, stockItem in
