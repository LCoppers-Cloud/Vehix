...
class InventoryTemplateUtility {
        ...
        for item in items {
            ...
            if stockLocations.isEmpty {
                ...
                let row = [
                    ...
                    String(format: "%.2f", item.pricePerUnit),
                    "", // No location
                ...
                for stockLocation in stockLocations {
                                    // Use locationName computed property for display purposes
                let row = [
                    escapeCsvField(item.name),
                    escapeCsvField(item.category),
                    escapeCsvField(item.itemDescription ?? ""),
    ...
    static func importInventoryFromCSV(data: Data, modelContext: ModelContext) throws -> [AppInventoryItem] {
        ...
        for column in requiredColumns {
            guard header.contains(where: { $0.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == column.lowercased() }) else {
                throw TemplateError.invalidFormat
            }
        }
    ...
    private static func getDefaultWarehouse(modelContext: ModelContext) throws -> AppWarehouse {
        ...
        let defaultWarehouse = AppWarehouse(name: "Main Warehouse", location: "Default Location", warehouseDescription: "Default warehouse created for inventory import")
