...
public struct StaffListView: View {
    ...
    @Query(sort: [SortDescriptor<AuthUser>(\.fullName)]) private var users: [AuthUser]
    @Query private var vehicleAssignments: [VehicleAssignment]

    // Sheet presentation management
    enum SheetType: String, Identifiable {
        case invite, editStaff
    ...
    // Constructor for ManagerDashboardView
    public init() {
            ...
            LazyVStack(spacing: 12) {
                ForEach(Array(filteredUsers.enumerated()), id: \.offset) { index, user in
                    StaffCardView(
                        ...
                        isEditing: isEditing,
                        onEdit: { 
                            // Close any other sheets first
                            activeSheet = .editStaff
                            editingUser = user 
                        },
