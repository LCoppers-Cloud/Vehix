...
struct PurchaseOrderRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                ...
                StatusBadge(
                    text: purchaseOrder.poStatus.rawValue,
                    color: statusColor(for: purchaseOrder.poStatus),
                    backgroundColor: statusColor(for: purchaseOrder.poStatus)
                )
            }
    ...
    private func statusColor(for status: PurchaseOrderStatus) -> Color {
        ...
        case .partiallyReceived: return Color.orange
