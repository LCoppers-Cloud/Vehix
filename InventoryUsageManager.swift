...
class InventoryUsageManager: ObservableObject {
    ...
    func recordItemUsage(
        ...
        // Create usage record using the InventoryUsageRecord from InventoryItemExtended.swift
        // NOT the AppInventoryUsageRecord type alias which has a different structure
        let usageRecord = InventoryUsageRecord(
            ...
            comments: comments,
            imageData: imageData,
            cost: item.pricePerUnit * Double(quantity),
            inventoryItem: item,
            vehicle: vehicle,
    ...
    func generateUsageReport(
        ...
        do {
            ...
            // Use InventoryUsageRecord from InventoryItemExtended.swift instead of AppInventoryUsageRecord
    ...
    private func processUsageRecords(_ records: [InventoryUsageRecord]) -> [UsageReportItem] {
        // Group records by item using the itemId from InventoryUsageRecord
        let groupedByItem = Dictionary(grouping: records) { record in record.itemId }
        ...
        return groupedByItem.map { itemId, records in
            ...
            let firstItem = records.first
            let itemName = firstItem?.inventoryItem?.name ?? "Unknown Item"
...
struct UsageReportItem: Identifiable {
    ...
    var records: [InventoryUsageRecord]  // Use InventoryUsageRecord instead of AppInventoryUsageRecord
} 